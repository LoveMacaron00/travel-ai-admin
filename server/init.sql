...
-- Smart Travel Diary ของผู้ใช้ โดยอ้างอิงสถานที่จริงเมื่อจับคู่ได้
CREATE TABLE IF NOT EXISTS travel_diary_entries (
    ...
    CONSTRAINT travel_diary_image_urls_array
        CHECK (jsonb_typeof(image_urls) = 'array'),
    CONSTRAINT travel_diary_source
        CHECK (source IN ('manual', 'gps', 'aiCamera', 'imported'))
);
...
CREATE TABLE IF NOT EXISTS chat_messages (
    ...
    image_path TEXT, -- ชื่อไฟล์ private ของรูปที่ผู้ใช้ส่งเข้า AI Camera
    image_url TEXT, -- URL รูปประกอบจากสถานที่ที่ AI จับคู่ได้
    image_mime_type VARCHAR(20),
    image_caption TEXT,
    edited_at TIMESTAMPTZ, -- มีค่าเมื่อผู้ใช้แก้ไขข้อความของตนเอง
    reply_to_message_id INT REFERENCES chat_messages(id) ON DELETE CASCADE,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
